...
public:
	...
	inline Animation& AddAnimation(const std::string& name, const Animation& animation) { return (m_animations[name] = animation); }
	inline Animation& AddAnimation(const std::string& name, Animation&& animation) { return (m_animations[name] = std::move(animation)); }
